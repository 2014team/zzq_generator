/*列表数据*/
const LIST = getAminUrl('admin/CENTER/GENERATETEMPLATES/LIST')
/*列表删除*/
const DELETE = getAminUrl('admin/CENTER/GENERATETEMPLATES/DELETE');
/*编辑*/
const EDIT = getAminUrl('admin/CENTER/GENERATETEMPLATES/EDIT');
/*批量删除*/
const BATCH_DELETE = getAminUrl('admin/CENTER/GENERATETEMPLATES/BATCH/DELETE');
/*查找单个对象*/
const GET = getAminUrl('admin/CENTER/GENERATETEMPLATES/GET');
//行对象
var rowObj = "";

const GENERATETEMPLATES = getAminUrl('admin/CENTER/GENERATETEMPLATES/EXECUTE');
...
function generator() {
	...
	layer.confirm('确认要生成吗？', function(index) {
		...
		reqPostHasParameter(GENERATETEMPLATES, {"idArr":array},function(result) {
			if (result.code == 200) { //这个是从后台取回来的状态值
				layer.msg(result.msg, {
					icon : 1,
					time : 1000
...
function updateRowData(obj){
	var reqData = obj.field;
	 reqPostHasParameter(GET, {"id":reqData.id}, function(result) {
		 ...
		 rowObj.update({
				 uniteConfigId: reqData.uniteConfigId,
				 uniteConfigIdName: reqData.uniteConfigIdName,
				 tableName: reqData.tableName,
				 createDate: reqData.createDate,
				 updateDate: reqData.updateDate,
				 pathParam: reqData.pathParam,
				 extendParam: reqData.extendParam,
			});	
	 }, function(e) {
		 console.log(e);
	 })
